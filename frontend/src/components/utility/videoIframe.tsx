...
export default function VideoIframe({ videoId, coverImg, size, inlinePlay, largeVideo, iconCenter }: Props) {
    ...
    return (
        <div className="relative h-full">
            <div className={`videoHolder h-full ${!isPlaying? 'after:content-[""] after:absolute after:top-0 after:left-0 after:right-0 after:bottom-0 after:z-35': ''} ${inlinePlay ? 'after:bg-black-grad' : 'after:bg-transparent'} ${largeVideo ? 'min-h-97.5' : ''}`}>
                {extractedVideoId && (
                    <YoutubePlayer
